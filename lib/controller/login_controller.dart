...
class AuthController extends GetxController {
  ...
  Future<void> login(BuildContext context,String username, String password, Route nextroute) async {
    final url = Uri.parse('$baseUrl/customer/login?platform=app');

    final response = await http.post(
      url,
      body: json.encode({'user_input': username, 'password': password}),
    ...
    if (response.statusCode == 200 && responseData["status"] == true) {
      ...
      await prefs.setString('user_customer_name', responseData['data']['customer_name']);
